...
BinaryHeap.prototype.insert = function (value) {
  ...
  for (i = len -1; i >=0; i--){
    ...
    // if (curr === value){
      if (curr < parent){
        this._heap[i] = parent;
        this._heap[parentIndex] = curr;
        console.log('swap', this._heap);
      }
    // }
  }
  // maybe need to sort the children
  for (var j = 0; j < len; j++){
    let curr = this._heap[j];
    let childrenIndeces = [j * 2 + 1, j * 2 + 2]
    let child1 = this._heap[childrenIndeces[0]];
    let child2 = this._heap[childrenIndeces[1]];
    // console.log(curr, child1, child2);
    if (child1 > child2){
      this._heap[childrenIndeces[0]] = child2;
      this._heap[childrenIndeces[1]] = child1;
    }
    // console.log('current is', curr);
    // console.log('child is', child);
    // console.log(minChild);
    if (curr > this._heap[childrenIndeces[0]]){
      this._heap[j] = this._heap[childrenIndeces[0]];
      this._heap[childrenIndeces[0]] = curr;
      // console.log('swap', this._heap);
    }
  }
...
BinaryHeap.prototype.removeRoot = function () {
  // TODO: Your code here
  var root = this._heap.shift();
  const len = this._heap.length;
  for (i = 0; i < len; i++){
    let curr = this._heap[i];
    let childrenIndeces = [i * 2 + 1, i * 2 + 2]
    let child1 = this._heap[childrenIndeces[0]];
    let child2 = this._heap[childrenIndeces[1]];
    let minChild;
    let minChildIndex;
    ...
    // console.log(minChild);
    if (curr > minChild){
      this._heap[i] = minChild;
      this._heap[childrenIndeces[minChildIndex]] = curr;
      // console.log('swap', this._heap);
    }
  }
  return root;
