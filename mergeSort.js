...
var mergeSortedArrays = function(array1, array2){
  ...
  var secondCount = 0;
  //console.log('running');
  while (ans.length < array1.length + array2.length){
    //check if the count exceeds the length of the array and if so add the rest of the other array to ans and break the loop
    //console.log('inside the while loop', firstCount, secondCount);
    if (firstCount >= array1.length){
      var restOfArray2 = array2.slice(secondCount,array2.length);

      //console.log(restOfArray2);
      return ans.concat(restOfArray2);
      break;
    }
    if (secondCount >= array2.length){
      var restOfArray1 = array1.slice(firstCount,array1.length+1);
      return ans.concat(restOfArray1);
      break;
    }
    let curr1 = array1[firstCount];
    let curr2 = array2[secondCount];
...
var mergeSort = function(array) {
  ...
  var secondHalf = mergeSort(array.slice(half, array.length));
  return mergeSortedArrays(firstHalf, secondHalf);
};

//console.log(mergeSortedArrays([1,2,4,6],[1,3,4,5]));
console.log(mergeSort([1,3,4,2,5,9,13,2,6]));