...
var shuffleDeck = function(deck) {
  ...
  var shuffledDeck = [];
  var len = deck.length;
  var initialLength = deck.length;
  while(shuffledDeck.length < initialLength){
    var randomVal = Math.floor(Math.random() * len);
    var curr = deck[randomVal];
    //console.log(randomVal, curr, len);
    shuffledDeck.push(curr);
    len --;
    deck.splice(randomVal, 1);
    //console.log(deck);
  }
